...
import vn.com.freesoft.docmanagement.controller.BackupController;
...
public class BackupDB {
	...
	public boolean backupDB(String dumpExePath, String host, String dbName, int port, String filePath) {
		...
		try {
			...
			if (connection.getPassword().length() > 0) {
				...
				// --add-drop-database -B "
				// + dbName + " -r \"" + filePath + "\"";
				batchCommand = dumpExePath + " -h " + host + " --port " + port + " -u " + connection.getUser() + " -p"
						+ connection.getPassword() + " --add-drop-database -B " + dbName + ">\"" + filePath + "\"";
			} else {
				batchCommand = dumpExePath + " -h " + host + " --port " + port + " -u " + connection.getUser()
						+ " --add-drop-database -B " + dbName + ">\"" + filePath + "\"";
			}
			...
			// p = runtime.exec("C:/Program Files/MySQL/MySQL Server 5.6/bin" +
			// batchCommand);
	...
	public boolean restoreDB(String source) {
		...
		String[] executeCmd = new String[] { "mysql", "--user=" + connection.getUser(),
				"--password=" + connection.getPassword(), "-e", "source " + source };

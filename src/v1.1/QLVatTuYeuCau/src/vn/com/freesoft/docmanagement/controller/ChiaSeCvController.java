...
import vn.com.freesoft.docmanagement.dao.VTCongVanDAO;
import vn.com.freesoft.docmanagement.dao.VaiTroDAO;
...
import vn.com.freesoft.docmanagement.entity.VaiTro;
...
public class ChiaSeCvController extends HttpServlet {
	...
	protected ModelAndView cscvManage(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (authentication == null) {
				...
				logger.error("Không có quy�?n chia sẻ công văn");
				return new ModelAndView(siteMap.login);
			}
	...
	protected ModelAndView chiaSeCv(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			NhatKy nhatKy = new NhatKy(
					nguoiDung.getMsnv(), congVan.getCvId() + "#Chia sẻ công văn số " + congVan.getSoDen()
							+ " nhận ngày " + DateUtil.toString(congVan.getCvNgayNhan()),
					currentDate, hotens.toString());
	...
	public @ResponseBody String preUpdateYeuCau(@RequestParam("msnv") String msnv, HttpServletRequest request,
	...
	public @ResponseBody String updateYeuCau(@RequestParam("vaiTroList") String vaiTroList, HttpServletRequest request,
		...
		try {
			...
			Date currentDate = DateUtil.convertToSqlDate(new java.util.Date());
			NhatKy nhatKy = new NhatKy(nguoiDung.getMsnv(), "chia sẻ công văn số " + congVan.getSoDen() + " nhận ngày "
					+ DateUtil.toString(congVan.getCvNgayNhan()), currentDate, hotens.toString());
			NhatKyDAO nhatKyDAO = new NhatKyDAO();
	...
	@RequestMapping(value = "/timKiemNguoidungCs", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
	public @ResponseBody String timKiemNguoidungCs(@RequestParam("msnv") String msnv,
			@RequestParam("hoTen") String hoTen, HttpServletRequest request) {
		try {
			HttpSession session = request.getSession(false);
			NguoiDung authentication = (NguoiDung) session.getAttribute("nguoiDung");
			String adminMa = context.getInitParameter("adminMa");
			...
			logger.error("NullPointer Exception khi phân tìm ngư�?i chia sẻ: " + e.getMessage());
			return JSonUtil.toJson("authentication error");
		}
	}

...
import vn.com.freesoft.docmanament.dao.CTVatTuDAO;
import vn.com.freesoft.docmanament.dao.ChatLuongDAO;
import vn.com.freesoft.docmanament.dao.CongVanDAO;
import vn.com.freesoft.docmanament.dao.NhatKyDAO;
import vn.com.freesoft.docmanament.dao.NoiSanXuatDAO;
...
public class YcController extends HttpServlet {
	...
	public ModelAndView manageYcvt(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (authentication == null) {
				...
				if (cdMa.equals(truongPhongMa)) {
					logger.error("Không có quy�?n cập nhật vật tư thiếu");
					return new ModelAndView(siteMap.login);
				}
			}
	...
	@RequestMapping(value = "/searchCtvt", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
	public @ResponseBody String searchCtvt(@RequestParam("vtMa") String vtMa, @RequestParam("vtTen") String vtTen,
			@RequestParam("nsx") String nsx, @RequestParam("chatLuong") String chatLuong, HttpServletRequest request) {
		try {
			HttpSession session = request.getSession(false);
			NguoiDung authentication = (NguoiDung) session.getAttribute("nguoiDung");
			String truongPhongMa = context.getInitParameter("truongPhongMa");
			if (authentication == null) {
				...
				String cdMa = authentication.getChucDanh().getCdMa();
				if (cdMa.equals(truongPhongMa)) {
					logger.error("Không có quy�?n truy cập cập nhật vật tư thiếu");
					return JSonUtil.toJson("authentication error");
				}
			}
			CTVatTuDAO ctvtDAO = new CTVatTuDAO();
	...
	@RequestMapping(value = "/preAddSoLuong", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
	...
	@RequestMapping(value = "/addSoLuong", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
	public @ResponseBody String addSoLuong(@RequestParam("soLuong") String soLuong, HttpServletRequest request) {
		try {
			...
			String content = "Vât tư có mã " + ctVatTu.getVatTu().getVtMa() + ", mã nơi sản xuất "
					+ ctVatTu.getNoiSanXuat().getNsxMa() + " và mã chất lượng " + ctVatTu.getChatLuong().getClMa()
					+ " được thêm tất cả " + yeuCau.getYcSoLuong();
			...
			NhatKy nhatKy = new NhatKy(authentication.getMsnv(),
					cvId + "#Thêm số lượng vật tư thiếu của công văn  có số đến " + congVan.getSoDen() + " nhận ngày "
							+ congVan.getCvNgayNhan(),
					currentDate, content);
			nhatKyDAO.addNhatKy(nhatKy);
			nhatKyDAO.disconnect();
			// NhatKy nhatKy = (NhatKy) session.getAttribute("nhatKy");
			// if (nhatKy == null) {
			// java.sql.Date currentDate = DateUtil.convertToSqlDate(new
			// java.util.Date());
			// nhatKy = new NhatKy(authentication.getMsnv(), currentDate, cvId +
			// "#Bạn đã cập nhật vật tư thiếu cho công văn có số đến " +
			// congVan.getSoDen() + " nhận ngày " + congVan.getCvNgayNhan() +
			// ":<br> ");
			// }
	...
	@RequestMapping(value = "/deleteYc", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
	public @ResponseBody String deleteYc(@RequestParam("ycList") String ycList, HttpSession session) {
		try {
			...
			for (String s : ycIdList) {
				...
				content.append("<br>&nbsp;&nbsp;+ Mã vật tư" + ctVatTu.getVatTu().getVtMa() + ", mã nơi sản xuất"
						+ ctVatTu.getNoiSanXuat().getNsxMa() + ", mã chất lượng " + ctVatTu.getChatLuong().getClMa()
						+ ".");
			...
			NhatKy nhatKy = new NhatKy(
					authentication.getMsnv(), cvId + "#" + "#Xóa vật tư thiếu của công văn  có số đến "
							+ congVan.getSoDen() + " nhận ngày " + congVan.getCvNgayNhan(),
					currentDate, content.toString());
			nhatKyDAO.addNhatKy(nhatKy);
			nhatKyDAO.disconnect();
			ycDAO.disconnect();
	...
	@RequestMapping(value = "/preUpdateYc", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
	...
	@RequestMapping(value = "/updateSoLuong", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
	public @ResponseBody String updateSoLuong(@RequestParam("soLuong") String soLuong, HttpServletRequest request) {
		try {
			...
			CongVanDAO congVanDAO = new CongVanDAO();
			CongVan congVan = congVanDAO.getCongVan(cvId);
			StringBuilder content = new StringBuilder("Vật tư có mã " + ctVatTu.getVatTu().getVtMa()
					+ ", mã nơi sản xuất " + ctVatTu.getNoiSanXuat().getNsxMa() + ", mã chất lượng "
					+ ctVatTu.getChatLuong().getClMa() + "của công văn có số đến " + congVan.getSoDen() + " nhận ngày "
					+ congVan.getCvNgayNhan() + " được đã được cập nhật " + yeuCau.getYcSoLuong());
			String dvt = ctVatTu.getVatTu().getDvt().getDvtTen();
			if (dvt.length() > 0)
				content.append("/" + dvt);
			...
			java.sql.Date currentDate = DateUtil.convertToSqlDate(new java.util.Date());
			NhatKy nhatKy = new NhatKy(
					authentication.getMsnv(), cvId + "#Thay đổi số lượng vật tư thiếu của công văn có số đến "
							+ congVan.getSoDen() + " nhận ngày " + congVan.getCvNgayNhan(),
					currentDate, content.toString());
			nhatKyDAO.addNhatKy(nhatKy);
			nhatKyDAO.disconnect();
	...
	@RequestMapping(value = "/preCapVatTu", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
	...
	@RequestMapping(value = "/capVatTu", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
	...
	@RequestMapping(value = "/loadPageCtvtYc", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
	...
	@RequestMapping(value = "/searchCtvtYc", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)

...
-->
					...
					<table class="tableContent" <%if (count % 2 == 1){ out.println("style=\"background : #CCFFFF; width: 100%; font-size: 18px;  \"");}else{out.println("style=\"background : #FFFFFF; width: 100%; font-size: 18px;\"");}%> class="border-congvan">
						...
						<% if (chucDanhMa.equals(vanThuMa) || chucDanhMa.equals(adminMa) || chucDanhMa.equals(thuKyMa)) {%>
							<td class="column-check" <%if (chucDanh.equals(adminMa)) out.println("rowspan=\"11\""); out.println("rowspan=\"10\"");%>  style="margin-right: 30px;">Chọn <input title="Click để chọn công văn"type="checkbox" name="cvId" value="<%=congVan.getCvId()%>"> 
							</td>
							
							<%} %>
							<td  style="font-weight: bold; width: 20%;x-icon" colspan="1">Số nhận: </td>	
							<td  style="text-align: left; width: 25%;"><%=congVan.getSoDen() %></td>
...
<%-- 									<%} %> --%>
								...
								<tr>
									...
									<%if (capPhat || chucDanhMa.equals(adminMa)) { %>
										<td colspan="1" style="float: right;">
											<button  class="button" type="button" style="width: 180px; height: 30px;" onclick="location.href='<%=siteMap.ycvtManage + "?cvId=" + congVan.getCvId()%>'">
						...
						<tr>
							...
							<% if(chucDanh.equals(truongPhongMa) || chucDanh.equals(vanThuMa)  || chucDanh.equals(adminMa)  || chucDanh.equals(phoPhongMa)) { %>
							<td colspan="1" style="float: right;">
										<button  class="button" type="button" style="width: 180px; height: 30px;" onclick="location.href='<%=siteMap.cscvManage + "?action=chiaSeCv&congVan=" + congVan.getCvId()%>'">

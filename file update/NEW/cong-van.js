...
function preUpdateCv(cv) {
	...
	//	  		$('#update-form input[name=file]').val(fileName);
		  		...
		  		$('#update-form textarea[name=trichYeuUpdate]').val(congVan.trichYeu.split("<br>").join("\n"));
		  		$('#update-form textarea[name=butPheUpdate]').val(congVan.butPhe.split("<br>").join("\n"));
	...
	//	  		$('#linkCv').html('<a href=\"' + path + '\" target=\"_blank\">Xem công văn</a>')
		  		...
		  		if (file != null) {
					...
					$('#update-form textarea[name=moTa]').val(file.moTa.split("<br>").join("\n"));
		  		} else {
		  			$('#linkCv').attr('href', '');
		  			$('#update-form textarea[name=moTa]').val('Không có ghi chú');
		  		}
		  		$('#linkCv').html(fileNameFull);
		  		//$('#update-form textarea[name=butPheUpdate]').val(congVan.butPhe.replace("<br>","\n"));
...
//						+ '<td colspan=\"1\" style=\"color:red;font-weight: bold;font-style: oblique;\">' + congVan.trangThai.ttTen + '</td>'
						...
						+ '<td style=\"text-align: left;color:red;\">' + congVan.cvSo + '</td>'
						+ '<th style=\"font-weight: bold; text-align: left;\">Ngày công văn đến: </th>'
						+ '<td style=\"text-align: left;color:blue;\">' + cvNgayDi+ '</td>'
						+ '</tr>'
						+ '<tr>'
						+ '<td style=\"font-weight: bold;\">Mục đích: </td>'
						+ '<td style=\"text-align: left\">' + congVan.mucDich.mdTen + '</td>'
						+ '<td style=\"font-weight: bold;\">Nơi gửi: </td>'
						+ '<td style=\"text-align: left\">' +  congVan.donVi.dvTen + '</td>'
						
						+ '</tr>'
						+ '<tr>'
						+ '<td style=\"font-weight: bold;\">Nội dung công tác: </td>'
						+ '<td colspan=\"3\" style=\"text-align: left;font-weight: bold;\">' +  congVan.trichYeu + '</td>'
						+ '</tr>'
						+ '<tr>'
						+ '<td style=\"font-weight: bold;\">Bút phê: </td>'
						+ '<td colspan=\"3\">' +  congVan.butPhe + '</td>'
						+ '</tr>';
//						+ '<tr>'
//						+ '<td class=\"left-column-first\" style=\"font-weight: bold;\">Nơi GQ chính</td>'
//						+ '<td class=\"column-color\" colspan=\"3\">' + congVan.donVi.dvTen + '</td>'
						+ '</tr>'
						//+ '<tr>'
						//+ '<td style=\"font-weight: bold;\">Nơi GQ chính</td>'
						//+ '<td>' + congVan.donVi.dvTen + '</td>'
//						+ '<td colspan=\"3\" style=\"float: right;\">'
//						+ '<button style=\"padding-left: 10px; padding-right: 10px;\"  class=\"button\" id=\"chiaSe\" type=\"button\" style=\"width: 170px; height: 30px;\"' 
//						+ '  onclick=\"location.href=\'/QLVatTuYeuCau/cscvManage.html?action=chiaSeCv&congVan=' + congVan.cvId + '\'\">'
//						+ '<i class=\"fa fa-spinner\"></i>&nbsp;&nbsp;Chia sẻ công văn'
//						+ '</button>'
//						+ '</td>'
...
//							}
						...
						tables += '<tr><td style=\"font-weight: bold;\">Người xử lý</td>'
						+ '<td colspan=\"3\">' + cellNguoiXl + '</td></tr>';
					...
					if (chucDanhMa == nhanVienMa || chucDanhMa == phoPhongMa || chucDanhMa == adminMa) { 
						...
						if (chucDanhMa != adminMa) {
							var cellVaiTro = '';
							tables += '<td style=\"font-weight: bold;\">Vai trò</td>'
								+ '<td class=\"column-color\" colspan=\"2\">';
							tables += '<table>';
	...
	//							cellVaiTro = vtCongVanList.vtTen.join(', ');
							...
							tables += '<th style=\"text-align: left;\">Vai trò</th><td colspan = \"2\"></td>';
						}	
							if (capPhat == true || chucDanhMa == adminMa) {
								tables += '<td colspan=\"1\" style=\"float: right;\">' 
								+ '<button  style=\"padding-left: 10px; padding-right: 10px;\" class=\"button\" type=\"button\" style=\"width: 180px; height: 30px;\" ' 
...
//						tables += s;
				...
				tables	+= '</tr>'
						...
						+ '<td colspan = \"3\">' + moTa + '</td>'
						+ '</tr>';
						
				tables += '<tr>' 
					+ '<th style="text-align: left"><label style="font-size: 18px;">Trạng thái</label></th>'
					+ '<td style=\"text-align: left; padding-left: 10px;\" colspan = \"2\" id =\"' + congVan.cvId  + 'ttMaCongVan\">';
				if (chucDanhMa == truongPhongMa || chucDanhMa == vanThuMa || chucDanhMa == adminMa || chucDanhMa == phoPhongMa) {
					tables += '<input type=\"radio\"' + ('CGQ'== congVan.trangThai.ttMa ? ' checked ' : '') + 'name=' + congVan.cvId  + ' value=\"' + congVan.cvId +'#' + 'CGQ\"' + ' class=\"ttMaUpdate\" >'; //onchange=\"changeTrangThai()\"
					tables += '&nbsp;<label for=\"' + congVan.cvId + '#CGQ\">Chưa giải quyết</label>&nbsp;&nbsp;&nbsp';
					tables += '<input type=\"radio\"' + ('DGQ'==congVan.trangThai.ttMa ? ' checked ' : '') + 'name=' + congVan.cvId + ' value=\"' + congVan.cvId +'#' + 'DGQ\"' + 'DGQ\" class=\"ttMaUpdate\">';
					...
					tables += '&nbsp;<label for=\"' + congVan.cvId + '#DaGQ\">Đã cấp đủ hàng</label>&nbsp;&nbsp;&nbsp</td>';
					tables += '<td colspan=\"1\" style=\"float: right;\">'
						+ '<button  class=\"button\" id=\"chiaSe\" type=\"button\" style=\"width: 180px; height: 30px;\" style=\"padding-left: 10px; padding-right: 10px;\" ' 
						+ '  onclick=\"location.href=\'/QLVatTuYeuCau/cscvManage.html?action=chiaSeCv&congVan=' + congVan.cvId + '\'\">'
						+ '<i class=\"fa fa-spinner\"></i>&nbsp;&nbsp;Chia sẻ công văn'
						+ '</button>'
						+ '</td>';
